...
describe('README pond demo', () => {
  it('embeds one native animated SVG containing every season and phase', () => {
    ...
    expect(svg.match(/@keyframes fp0/g)).toHaveLength(1)
    expect(svg).not.toContain('@keyframes rd0-fp0')
    expect(svg).toContain('url(#rd0-fx)')
    expect(svg.match(/<symbol id="readme-fish-/g)?.length).toBeGreaterThanOrEqual(1)
    expect(svg.match(/<use href="#readme-fish-/g)?.length).toBeGreaterThanOrEqual(8)
    expect(svg.match(/<use href="#readme-contributions"/g)).toHaveLength(8)
    expect(svg).not.toContain('data:image/svg+xml;base64,')
    expect(svg).not.toContain('<image')
    expect(Buffer.byteLength(svg)).toBeLessThan(700_000)
  })
})
