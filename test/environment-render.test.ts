...
describe('environment-aware original renderer', () => {
  ...
  it('uses physical seasonal markers while preserving the original pond language', () => {
    ...
    expect(springSvg).toContain('data-spring-kind="ribbon"')
    expect(springSvg).toContain('data-spring-kind="pondweed"')
    expect(springSvg).toContain('data-spring-kind="fanwort"')
    expect(springSvg.match(/class="spring-blade"/g)?.length).toBeGreaterThanOrEqual(11)
    expect(springSvg.match(/class="spring-tip"/g)?.length).toBeGreaterThanOrEqual(11)
    expect(springSvg.match(/data-spring-maturity="shoot"/g)).toHaveLength(2)
    expect(springSvg).toContain('data-spring-depth="back"')
    expect(springSvg).toContain('data-spring-depth="middle"')
    expect(springSvg).toContain('data-spring-depth="front"')
    expect(springSvg.match(/data-spring-root=/g)).toHaveLength(3)
    expect(springSvg.match(/data-spring-substrate=/g)).toHaveLength(3)
    expect(springSvg.match(/data-spring-foreground=/g)).toHaveLength(3)
    expect(springSvg).toContain('@keyframes spring-tip{')
    ...
    expect(summerSvg).toContain('data-lotus-petal-layout="lobed"')
    const lotusVariants = [...summerSvg.matchAll(/data-lotus-variant="(\d+)"/g)].map(match => match[1])
    expect(new Set(lotusVariants).size).toBeGreaterThanOrEqual(3)
    expect(summerSvg.match(/data-lotus-rotation=/g)?.length).toBeGreaterThanOrEqual(3)
    expect(summerSvg).toContain('data-lotus-pad="notched"')
    ...
    expect(winterSvg).toContain('data-seasonal-part="winter-snowfall"')
    expect(winterSvg.match(/data-snow-landing="ice"/g)?.length).toBeGreaterThanOrEqual(4)
    expect(winterSvg.match(/data-snow-landing="water"/g)?.length).toBeGreaterThanOrEqual(12)
    expect(winterSvg).toContain('class="snow-settle"')
    expect(winterSvg).toContain('class="snow-melt"')
  ...
  it('preserves seasonal identity at night and closes summer lotus flowers', () => {
    ...
    expect(renders[1].match(/data-lotus-bud-layout="folded-petals"/g)?.length).toBeGreaterThanOrEqual(3)
