...
const outputs = (process.env.KOIPOND_OUTPUTS ?? 'dist/koi-almanac-light.svg\ndist/koi-almanac-dark.svg?theme=dark')
  .split('\n')
  .map(line => line.trim())
  .filter(Boolean)
...
if (!generator) {
  console.error(
    'Cannot prove the generator version. Reference pillowtalk-Qy/koi-almanac with a full 40-character commit SHA.',
  )
  process.exit(1)
}
